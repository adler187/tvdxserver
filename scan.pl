...
$CONFIG_PROG = '/home/rjd/silicon_dust/libhdhomerun/hdhomerun_config';
...
$WGET = '/usr/bin/wget --quiet --output-document=- ';
...
$CACHE_FILE = '/home/rjd/silicon_dust/fcc_cache';

# Directory to write raw data for cricket to parse.  Filename is callsign.
$CRICKET_RAW_DIR = '/home/rjd/silicon_dust/cricket-raw';

# cricket config directory
$CRICKET_CONFIG = '/etc/cricket/config/TV';

# store last time, signal strength and quality when station was in
# in files like "$LAST_SEEN_DIR/wxyz"
$LAST_SEEN_DIR = '/home/rjd/silicon_dust/last_seen';

# javascript data file
$JS_DATA = '/var/www/TV/map_data.js';
...
while(1) {
  ...
  my $this_scan_file = "/home/rjd/silicon_dust/scans/scan.$file_time";
...
#      print SCAN_FILE "###### found call $1 channel $channel\n";
      $callsign{$channel} = $1;
...
# WFWA and WKAR channel 40 have collision
  ...
  foreach $channel (sort keys %callsign) {
    ...
    if (! exists $lat{$callsign}) {
      my $wget = "$WGET '$FCC_URL"
                 . "call=$callsign&chan=$channel&cha2=$channel&list=4&size=9'";
#      print SCAN_FILE "##### Trying $wget\n";
	...
	$lon{$callsign} = $lon_decimal;
        ...
        ($dx_km{$callsign},$azimuth{$callsign}) =
           dist("$lat_decimal $lon_decimal","$TUNER_LAT $TUNER_LON");
...
EOTARGET
  ...
  foreach my $last_seen_call_file (glob "$LAST_SEEN_DIR/*") {
    ...
    if ( exists $color_of_strongest_in{$location{$uc_last_seen_call}}
      && $color_of_strongest_in{$location{$uc_last_seen_call}} ne 'black'){next}
